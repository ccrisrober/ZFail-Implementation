...
void do_movement() {
	...
	if (keyRepeat[GLFW_KEY_M]) {
		((ShadowZScene*)scene)->addNewObject();
		keyRepeat[GLFW_KEY_M] = false;
	}
	if (keyRepeat[GLFW_KEY_N]) {
		((ShadowZScene*)scene)->removeObject();
...
void mainLoop() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (index == 0) {
			...
			strm.precision(4);
			strm << " [FPS: " << fps << "]";
			strm << " Num. objects: " << ((ShadowZScene*)scene)->objectsInScene();
...
int main(int argc, char** argv) {
	...
	scene = new ShadowZScene(WINDOW_WIDTH, WINDOW_HEIGHT , numObjs);
	scene->initScene();
